 ...
 * @param {{valueFlags?: string[], requireOperand?: boolean}} [options] -
 *   `valueFlags` lists flags whose space-separated value is the NEXT argv token
 *   (e.g. `--since 7`). Without this, a value that itself starts with `-` (e.g.
 *   `--since -5`, a negative day count) would be misread as an unrecognized
 *   flag rather than left for the caller's own value validation to reject with
 *   a clearer message. `--flag=value` is self-contained and never needs to be
 *   listed.
 *
 *   `requireOperand` turns a MISSING operand into a usage error here, before
 *   --help (#3087; the block comment at the check itself says why it is
 *   opt-in). Opt-in is a choice between two guards, never a way to have none:
 *   tests/validate-flags-value-operand.test.mjs sweeps every call site and
 *   fails CI unless the caller either passes `requireOperand: true` or is
 *   listed there with the richer validator it runs instead.
 ...
 *
 * CHANGING THIS SIGNATURE: tests/validate-flags-value-operand.test.mjs reads
 * call sites as TEXT — it locates `validateFlags(`, slices the arguments, and
 * inspects the 4th one for the names `valueFlags` and `requireOperand`.
 * Reordering the parameters, collapsing them into a single options object, or
 * renaming either property makes that sweep stop understanding the tree, so it
 * is part of any such diff. Most of those changes fail loudly; a few go
 * SILENTLY green while checking nothing. That test's header lists which is
 * which, measured case by case — read it before you change this line.
 */
