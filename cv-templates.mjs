...
// Resolve the career-ops checkout root. Core scripts run with the checkout as
// their cwd, but the 2.0 web app can point at a separate data checkout via
// CAREER_OPS_ROOT (see web/README + web/.env.local). Honor it so templates and
// the profile resolve against the user's actual files, not this module's own
// directory. Falls back to that directory, which is the checkout root in a
// normal install — keeping the standalone CLI and existing installs unchanged.
export function careerOpsRoot() {
  const env = process.env.CAREER_OPS_ROOT?.trim();
  return env ? resolve(env) : __dirname;
}

const DEFAULT_TEMPLATES_DIR = resolve(careerOpsRoot(), 'templates');
const DEFAULT_PROFILE_PATH =
  process.env.CAREER_OPS_PROFILE || resolve(careerOpsRoot(), 'config', 'profile.yml');
