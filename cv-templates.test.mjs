/**
 * cv-templates.test.mjs — tests for the CV / cover-letter template resolver.
 *
 * Run: node cv-templates.test.mjs
 */
import { test } from 'node:test';
import assert from 'node:assert/strict';
import { prettify, kebab, KINDS, careerOpsRoot } from './cv-templates.mjs';
...
test('careerOpsRoot: honors CAREER_OPS_ROOT, else an absolute local dir', () => {
  const prev = process.env.CAREER_OPS_ROOT;
  try {
    process.env.CAREER_OPS_ROOT = '/tmp/career-ops-checkout';
    assert.equal(careerOpsRoot(), '/tmp/career-ops-checkout');
    delete process.env.CAREER_OPS_ROOT;
    const fallback = careerOpsRoot();
    assert.ok(fallback.startsWith('/'), 'falls back to an absolute directory');
  } finally {
...
import { validateTemplate } from './cv-templates.mjs';
...
import { resolveTemplate, loadProfileDefault } from './cv-templates.mjs';
